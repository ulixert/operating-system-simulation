...
void handle_user_command(const char *command) {
    if (strcmp(command, "help") == 0) {
        ...
        printf("  run <time>    - Create a process\n");
        ...
        if (name && time_str) {
            ...
            if (time_required > 0) {
                ...
                create_process(pid_counter++, ppid, 1000, time_required, name);
            } else {
        ...
        char *message_start = strchr(command + 5, '"');
        if (message_start) {
        ...
        sys_list_directory(false);
    } else if (strcmp(command, "ls -a") == 0) {
        sys_list_directory(true);
